...
import { geminiSummariseCommit } from "./gemini";
...
async function summarizeCommit(githubUrl: string, commitHash: string) {
  const githubDiffUrl = `${githubUrl}/commit/${commitHash}.diff`;
  const { data } = await axios.get(githubDiffUrl, {
    headers: {
      Accept: `application/vnd.github.v3.diff`,
    },
  });

  return (await geminiSummariseCommit(data)) || "";
}
